...
public class RobotContainer {
	...
	private final SpeedController speedController =
		new SpeedController(SpeedLevel.DEFAULT);
	...
	public RobotContainer() {
		switch(Constants.getRobot()) {
			case COMP_BOT:
				...
				drive = new Drive(new GyroIOPigeon2(true),
					new ModuleIOSparkMax(
						DriveConstants.FRONT_LEFT_MODULE_CONFIG),
					new ModuleIOSparkMax(
						DriveConstants.FRONT_RIGHT_MODULE_CONFIG),
					new ModuleIOSparkMax(
						DriveConstants.BACK_LEFT_MODULE_CONFIG),
					new ModuleIOSparkMax(
						DriveConstants.BACK_RIGHT_MODULE_CONFIG));
			...
			case SIM_BOT:
				...
				flywheelExample = new Flywheel(new FlywheelIOSparkMax());
				vision = new AprilTagVision(new CameraIOSim(
					VisionConstants.FRONT_CAMERA, drive::getPose));
				break;
		...
		vision.addVisionEstimateConsumer((visionEstimate) -> {
			if(Constants.getRobot() != RobotType.SIM_BOT &&
				visionEstimate.isSuccess()) {
				drive.addVisionMeasurement(visionEstimate.robotPose2d(),
					visionEstimate.timestampSeconds(),
					visionEstimate.standardDeviations());
			}
		});

		// Can also use AutoBuilder.buildAutoChooser(); instead of
		// SendableChooser to auto populate
		autoChooser = new LoggedDashboardChooser<>(
			"Auto Chooser", new SendableChooser<Command>());
		...
		NamedCommands.registerCommand(
			"StopWithX", drive.runOnce(drive::stopUsingBrakeArrangement));
		NamedCommands.registerCommand(
			"Shoot", flywheelExample
						 .startEnd(()
									   -> flywheelExample.runVelocity(1000),
		...
		autoChooser.addOption("Four Note Center Choreo",
			new PathPlannerAuto("Four Note Center Choreo"));

		// Alerts for constants to avoid using them in competition
		if(Constants.TUNING_MODE) {
			new Alert("Tuning mode active, do not use in competition.",
				AlertType.INFO)
				.set(true);
		}
	...
	private void configureDriverControllerBindings() {
		...
		SmartDashboard.putData(
			Commands.runOnce(drive::zeroGyro).withName("Zero Gyro"));
		...
		if(driverController instanceof CommandXboxController) {
			...
			final Trigger useFieldRelative =
				new Trigger(new OverrideSwitch(driverXbox.y(), "Field Relative",
					OverrideSwitch.Mode.TOGGLE, true));

			final Trigger useAngleControlMode =
				new Trigger(new OverrideSwitch(driverXbox.rightBumper(),
					"Angle Driven", OverrideSwitch.Mode.HOLD, true));

			// Controllers
			final TeleopDriveController input = new TeleopDriveController(drive,
				()
					-> - driverXbox.getLeftY(),
				()
					-> - driverXbox.getLeftX(),
				() -> - driverXbox.getRightY(), () -> - driverXbox.getRightX());

			final HeadingController headingController =
				new HeadingController(drive);
					...
					.runEnd(
						()
							...
							drive.setRobotSpeeds(
								speedController.applyTo(new ChassisSpeeds(
			...
			for(int pov = 0; pov < 360; pov += includeDiagonalPOV ? 45 : 90) {
				...
				// POV angles are in Clock Wise degrees, needs to be flipped to
				// get correct rotation2d
				final Rotation2d angle = Rotation2d.fromDegrees(-pov);
				final String name = String.format("%d\u00B0", pov);

				// While the POV is being pressed and we are not in angle
				// control mode, set the chassis speeds to the Cos and Sin of
				// the angle
				driverXbox.pov(pov)
								...
								()
									-> drive.setRobotSpeeds(
										speedController.applyTo(
											new ChassisSpeeds(angle.getCos(),
												angle.getSin(), 0))),
								drive::stop)
							.withName(
								String.format("DriveRobotRelative %s", name)));

				// While the POV is being pressed and we are angle control mode
				// Start by resetting the controller and setting the goal angle
				// to the pov angle
				driverXbox.pov(pov)
					...
					.onTrue(drive
								...
								.withName(String.format(
									"PrepareLockedHeading %s", name)));

				// Then if the button is held for more than 0.2 seconds, drive
				// forward at the angle once the chassis reaches it
				driverXbox.pov(pov)
					.debounce(0.2)
					.and(useAngleControlMode)
					.whileTrue(
						drive
							.run(() -> {
								...
								drive.setRobotSpeeds(
									speedController.applyTo(new ChassisSpeeds(
										(headingController.atGoal() ? 1 : 0), 0,
										headingController.atGoal() ?
											0 :
											rotationRadians)));
							})
							.withName(String.format(
								"ForwardLockedHeading %s", name)));

				// Then once the pov is let go, if we are not at the angle
				// continue turn to it, while also accepting x and y input to
				// drive
				driverXbox.pov(pov)
					.and(useAngleControlMode)
					.onFalse(
						drive
							...
							.until(()
									   -> input.getOmegaRadiansPerSecond()
												  .getRadians() != 0)
							.withName(
								String.format("DriveLockedHeading %s", name)));
			}

			// While X is held down go into stop and go into the cross position
			// to resistent movement, then once X button is let go put modules
			// forward
			driverXbox.x().whileTrue(
				drive
					.startEnd(drive::stopUsingBrakeArrangement,
						drive::stopUsingForwardArrangement)
					.withName("StopWithX"));

			// When be is pressed stop the drivetrain then idle it, cancelling
			// all incoming commands. Also do this when robot is disabled
			driverXbox.b()
				.or(RobotModeTriggers.disabled())
				.whileTrue(drive.runOnce(drive::stop)
							   .andThen(Commands.idle(drive))
							   .withInterruptBehavior(
								   InterruptionBehavior.kCancelIncoming)
							   .withName("StopCancel"));

			// When right (Gas) trigger is held down or left stick (sprint) is
			// pressed, put in boost (fast) mode
			...
			// When left (Brake) trigger is held down or right stick (crouch) is
			// pressed, put in precise (slow) mode
			driverXbox.leftTrigger(0.5)
				...
				.whileTrue(Commands.startEnd(
					...
					()
						-> speedController.removeSpeedLevel(
							SpeedLevel.PRECISE)));

		} else if(driverController instanceof CommandJoystick) {
			final CommandJoystick driverJoystick =
				(CommandJoystick)driverController;

			TeleopDriveController input = new TeleopDriveController(drive,
				()
					-> - driverJoystick.getY(),
				()
					-> - driverJoystick.getX(),
				() -> - driverJoystick.getTwist(), () -> 0);
	...
	private void configureOperatorControllerBindings() {
		...
		if(operatorController instanceof CommandXboxController) {
			...
			// Shoot
			operatorXbox.leftTrigger().whileTrue(Commands.startEnd(
				()
					-> flywheelExample.runVelocity(
						3000 + robotState.flywheelShootRPMCompensation),
				flywheelExample::stop, flywheelExample));
		}
	}
	...
	public void updateSmartDashboardOutputs() {
		...
		SmartDashboard.putNumber(
			"Heading Degrees", -pose.getRotation().getDegrees());
		SmartDashboard.putNumber("Speed MPH",
			Math.hypot(speeds.vxMetersPerSecond, speeds.vyMetersPerSecond) *
				2.2369);

		SmartDashboard.putString(
			"Speed Level", speedController.getCurrentSpeedLevel().name());
		SmartDashboard.putString("Speed Transl",
			String.format("%.2f%%", speedController.getCurrentSpeedLevel()
