 ...
 *
 * <p>Module IO implementation for Talon FX drive motor controller, Talon FX
 * turn motor controller, and CANcoder. Specifically for Krakens motors with
 * Talons, with added power and efficiency benefits of Field Oriented Control
 * (FOC)
 *
 * <p>To calibrate the absolute encoder offsets, point the modules straight
 * (such that forward motion on the drive motor will propel the robot forward)
 * and copy the reported values from the absolute encoders using AdvantageScope.
 * These values are logged under
...
public class ModuleIOKrakenFOC implements ModuleIO {
	...
	private final TalonFXConfiguration turnTalonConfig =
		new TalonFXConfiguration();
	...
	public ModuleIOKrakenFOC(ModuleConfig config) {
		...
		// Gear Ratio
		driveTalonConfig.Feedback.SensorToMechanismRatio =
			MODULE_CONSTANTS.driveReduction();
		turnTalonConfig.Feedback.SensorToMechanismRatio =
		...
		BaseStatusSignal.setUpdateFrequencyForAll(
			DriveConstants.ODOMETRY_FREQUENCY_HERTZ, drivePosition,
			turnPosition);
		BaseStatusSignal.setUpdateFrequencyForAll(50.0, driveVelocity,
			driveAppliedVolts, driveCurrent, driveTorqueCurrent,
	...
	public void updateInputs(ModuleIOInputs inputs) {
		...
		inputs.driveTorqueCurrentAmps =
			driveTorqueCurrent.getAppliedUpdateFrequency();
	...
	public void setDriveVelocity(
		double velocityRadsPerSec, double feedForward) {
		driveTalon.setControl(new VelocityDutyCycle(velocityRadsPerSec)
	...
	public void setTurnPosition(double angleRads) {
		driveTalon.setControl(
			new VelocityDutyCycle(Units.radiansToRotations(angleRads))
				.withUpdateFreqHz(0));
	}
	...
	public void setDriveBrakeMode(boolean enable) {
		// Annoyingly complicated way, but we don't want to block main thread,
		// and we can't have two threads writing at the same time
