...
public class TeleopDriveController {
	...
	private static final LoggedTunableNumber stickDeadband =
		group.build("Deadband", 0.2);
	...
	private final DoubleSupplier xSupplier, ySupplier, xAngleSupplier,
		yAngleSupplier;
	 ...
	 */
	public TeleopDriveController(Drive drive, DoubleSupplier xSupplier,
		DoubleSupplier ySupplier, DoubleSupplier xAngleSupplier,
	...
	private static Translation2d getTranslationMetersPerSecond(double xInput,
		double yInput, double maxTranslationSpeedMetersPerSecond) {

		Translation2d translation = new Translation2d(xInput, yInput);

		// get length of linear velocity vector, and apply deadband to it for
		// noise reduction
		double magnitude =
			MathUtil.applyDeadband(translation.getNorm(), stickDeadband.get());

		// squaring the magnitude of the vector makes for quicker ramp up and
		// slower fine control, magnitude should always be positive
		double magnitudeSquared = Math.copySign(Math.pow(magnitude, 2), 1);

		// get a vector with the same angle as the base linear velocity vector
		// but with the magnitude squared
		Translation2d squaredLinearVelocity =
			new Pose2d(new Translation2d(), translation.getAngle())
				.transformBy(
					new Transform2d(magnitudeSquared, 0.0, new Rotation2d()))
				.getTranslation();
