...
public class HeadingController {
	...
	private static final LoggedTunableNumber toleranceDegrees =
		group.build("ToleranceDegrees", 1);
	...
	public HeadingController(Drive drive) {
		...
		headingControllerRadians = new ProfiledPIDController(Kp.get(), 0,
			Kd.get(), new TrapezoidProfile.Constraints(0.0, 0.0),
	...
	/**
	 * Set goal heading in radians. Calculate will now give values to get to
	 * this heading.
	 *
	...
	public double calculate() {
		...
		Logger.recordOutput("Drive/HeadingController/Goal",
			headingControllerRadians.getGoal().position);
		Logger.recordOutput("Drive/HeadingController/Output", output);
		Logger.recordOutput("Drive/HeadingController/HeadingError",
			headingControllerRadians.getPositionError());
		Logger.recordOutput("Drive/HeadingController/AtGoal",
			headingControllerRadians.atGoal());
	 ...
	 *
	 * @return true if the absolute value of the position error is less than
	 *     tolerance
	 */
