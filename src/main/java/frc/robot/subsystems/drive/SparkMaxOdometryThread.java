...
public class SparkMaxOdometryThread {
    ...
    /**
     * Starts the Notifier to periodically call the {@link #periodic} method at the frequency
     * defined by DriveConstants.ODOMETRY_FREQUENCY.
     *
     * <p>This method only starts the Notifier if there are timestamp queues registered to keep
     * track of time.
     */
    public void start() {
     ...
     * @return The queue where the sampled values will be stored. Read from this queue as values
     *     come in
     */
     ...
     *
     * <p>The method ensures that all signals are present before storing the values and timestamps.
     * If any signal is not present, no values or timestamps are stored.
     *
     * <p>This callback should be written so that it completes before the next time it's scheduled
     * to run in the notifier.
     */
