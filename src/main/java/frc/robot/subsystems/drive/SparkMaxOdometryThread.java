...
public class SparkMaxOdometryThread {
	...
	/**
	 * Returns the singleton instance of the SparkMaxOdometryThread.
	 ...
	 * Initializes the Notifier to periodically call the {@link #periodic} method.
	 */
	...
	public void start() {
		if (!timestampQueues.isEmpty()) {
			notifier.startPeriodic(1.0 / DriveConstants.ODOMETRY_FREQUENCY); // T = 1/f
		}
	}
	 ...
	 * @return The queue where the sampled values will be stored. Read from this queue as values come in
	 */
	...
	public Queue<Double> makeTimestampQueue() {
		...
		// timestamps will be added to each queue in list of timestamp queues each time periodic is called,
	...
	/**
	 * The method that is periodically called by the Notifier.
	 * It samples the signals and stores the values and corresponding timestamps
	 * in the respective queues.
	 *
	 * <p>
	 * The method ensures that all signals are present before storing the values
	 * and timestamps. If any signal is not present, no values or timestamps are stored.
	 *
	 * <p>
	 * This callback should be written so that it completes before the next time it's scheduled to run in the notifier.
	 */
