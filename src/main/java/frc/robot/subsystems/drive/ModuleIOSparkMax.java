 ...
 * motor controller (NEO or NEO 550), and CANCoder absolute encoder
 *
 ...
 * "/Drive/ModuleX/TurnAbsolutePositionRad". Update these values in
 * DriveConstants.
 */
public class ModuleIOSparkMax implements ModuleIO {
	...
	public ModuleIOSparkMax(ModuleConfig config) {
		...
		turnFeedback.setSmartMotionMaxAccel(
			MODULE_LIMITS_FREE.maxSteeringVelocity(), 0);
		...
		// Set update period
		double odometryPeriodSeconds =
			1.0 / DriveConstants.ODOMETRY_FREQUENCY_HERTZ;
	...
	public void updateInputs(ModuleIOInputs inputs) {
		...
		inputs.turnAppliedVolts =
			turnSparkMax.getAppliedOutput() * turnSparkMax.getBusVoltage();
	...
	public void setDriveVelocity(
		double velocityRadsPerSec, double feedForwardVoltage) {
		driveFeedback.setReference(velocityRadsPerSec, ControlType.kPosition, 0,
			feedForwardVoltage, ArbFFUnits.kVoltage);
	}
