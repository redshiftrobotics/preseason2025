...
/**
 * Swerve drivetrain (chassis) of robot. This contains four swerve modules and
 * a gyro
 */
public class Drive extends SubsystemBase {
	...
	public Drive(GyroIO gyroIO, ModuleIO flModuleIO, ModuleIO frModuleIO,
		...
		modules =
			new Module[] {new Module(flModuleIO,
							  new Translation2d(trackCenterX, trackCenterY)),
				new Module(
					frModuleIO, new Translation2d(trackCenterX, -trackCenterY)),
				new Module(
					blModuleIO, new Translation2d(-trackCenterX, trackCenterY)),
				new Module(brModuleIO,
					new Translation2d(-trackCenterX, -trackCenterY))};

		// --- Set up kinematics ---

		kinematics =
			new SwerveDriveKinematics(modules()
										  .map(Module::getDistanceFromCenter)
										  .toArray(Translation2d[] ::new));

		// --- Set up odometry ---

		lastModulePositions = modules()
								  .map(Module::getPosition)
								  .toArray(SwerveModulePosition[] ::new);
		poseEstimator = new SwerveDrivePoseEstimator(
			kinematics, rawGyroRotation, lastModulePositions, pose);
		...
		sysId = new SysIdRoutine(
			...
			new SysIdRoutine.Mechanism(voltage
				-> modules().forEach(
					(module)
						-> module.runCharacterization(voltage.in(Units.Volts))),
				null, this));
	}
	...
	public void periodic() {
		...
		Logger.recordOutput(
			"SwerveStates/MeasuredWheelSpeeds", getWheelSpeeds().states);
		Logger.recordOutput("SwerveStates/ModuleDesiredWheelSpeeds",
			getDesiredWheelSpeeds().states);

		// Update odometry
		double[] sampleTimestamps =
			modules[0].getOdometryTimestamps();// All signals are sampled
											   // together, use first
		...
		for(int i = 0; i < sampleCount; i++) {
			...
			for(int moduleIndex = 0; moduleIndex < modules.length;
				moduleIndex++) {
			...
			if(gyroInputs.connected) {
				...
				// Use the delta of swerve module to create estimated amount
				// twisted
			...
			pose = poseEstimator.updateWithTime(
	 ...
	 *     must use a timestamp with an epoch since FPGA time startup.
	 */
	 ...
	 *     This matrix is in the form [x, y, theta], with units in meters and
	 *     radians.
	 */
	public void addVisionMeasurement(Pose2d visionPose, double timestamp,
	 ...
	 *
	 * @return translational speed in meters/sec and rotation speed in
	 *     radians/sec
	 */
	 ...
	 *
	 * @param fieldRelative true if velocity is relative to field, false if
	 *     relative to chassis
	 * @return translational speed in meters/sec and rotation speed in
	 *     radians/sec
	 */
	...
	public void setRobotSpeeds(ChassisSpeeds speeds, boolean fieldRelative) {
		...
		speeds =
			ChassisSpeeds.discretize(speeds, Constants.LOOP_PERIOD_SECONDS);
	...
	/**
	 * Set desired swerve modules for each swerve module. Each wheel state is a
	 * turn angle and drive velocity in meters/second.
	 *
	 * @return a {@link SwerveDriveWheelStates} object which contains an array
	 *     of all desired swerve module states
	 */
	public void setWheelSpeeds(SwerveDriveWheelStates speeds) {

		SwerveDriveKinematics.desaturateWheelSpeeds(
			speeds.states, getMaxLinearSpeedMetersPerSec());
	...
	/**
	 * Get measured swerve module speeds for each swerve module. Each wheel
	 * state is a turn angle and drive velocity in meters/second.
	 *
	 * @return a {@link SwerveDriveWheelStates} object which contains an array
	 *     of all swerve module states
	 */
	public SwerveDriveWheelStates getWheelSpeeds() {
		return new SwerveDriveWheelStates(
			modules()
				.map(Module::getSpeeds)
				.toArray(SwerveModuleState[] ::new));
	}

	/**
	 * Get desired swerve module desired speeds for each swerve module. Each
	 * wheel state is a turn angle and drive velocity in meters/second.
	 *
	 * @return a {@link SwerveDriveWheelStates} object which contains an array
	 *     of all desired swerve module states.
	 */
	 ...
	 * position is a turn angle and drive position in meters
	 *
	 * @return a {@link SwerveDriveWheelPositions} object which contains an
	 *     array of all swerve module positions
	 */
	...
	/**
	 * Stops the drive and turns the modules to their forward position. The
	 * modules will return to their normal driving the next time a nonzero
	 * velocity is requested.
	 */
	public void stopUsingForwardArrangement() {
		Rotation2d[] headings = modules()
									.map(module -> new Rotation2d())
									.toArray(Rotation2d[] ::new);
		kinematics.resetHeadings(headings);
		setRobotSpeeds(new ChassisSpeeds());
	}
