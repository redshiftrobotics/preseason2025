 ...
 *
 * <p>Uses two flywheel sims for the drive and turn motors, with the absolute
...
public class ModuleIOSim implements ModuleIO {
	...
	private DCMotorSim driveSim = new DCMotorSim(
		DCMotor.getNEO(1), MODULE_CONSTANTS.driveReduction(), 0.025);
	private DCMotorSim turnSim = new DCMotorSim(
		DCMotor.getNEO(1), MODULE_CONSTANTS.turnReduction(), 0.004);
	...
	public void updateInputs(ModuleIOInputs inputs) {
		...
		inputs.odometryDrivePositionsRad =
			new double[] {inputs.drivePositionRad};
	...
	public void setDriveVelocity(
		...
		setDriveVoltage(
			driveFeedback.calculate(
				driveSim.getAngularVelocityRadPerSec(), velocityRadsPerSec) +
			feedForward);
	}
