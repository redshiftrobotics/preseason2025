...
public class Flywheel extends SubsystemBase {
	...
	public Flywheel(FlywheelIO io) {
		...
		ffModel = new SimpleMotorFeedforward(FEED_FORWARD_CONFIG.Ks(),
			FEED_FORWARD_CONFIG.Kv(), FEED_FORWARD_CONFIG.Ka());

		io.configurePID(PID_CONFIG.Kp(), PID_CONFIG.Ki(), PID_CONFIG.Kd());

		// Configure SysId
		sysId =
			new SysIdRoutine(new SysIdRoutine.Config(null, null, null,
								 state
								 -> Logger.recordOutput(
									 "Flywheel/SysIdState", state.toString())),
	...
	public void periodic() {
		...
		RobotState.getInstance().flywheelAccelerating =
			Math.abs(inputs.currentAmps) > 50.0 &&
			Math.abs(inputs.velocityRadPerSec) > 2;
	}
