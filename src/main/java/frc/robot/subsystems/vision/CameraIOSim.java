...
public class CameraIOSim implements CameraIO {
	...
	public CameraIOSim(
		CameraConfig config, Supplier<Pose2d> robotPoseSupplier) {
		...
		// These values depend on photonvision config, update them as well as in
		// assets config
	...
	public void updateInputs(CameraIOInputs inputs) {
		...
		if(estimatedRobotPoseOptional.isPresent()) {
			EstimatedRobotPose estimateRobotPose =
				estimatedRobotPoseOptional.get();
			...
			inputs.tagsUsed = estimateRobotPose.targetsUsed.stream()
