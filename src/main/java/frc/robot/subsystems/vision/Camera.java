...
public class Camera {
	...
	private static final LoggedTunableNumberGroup group = new LoggedTunableNumberGroup("VisionResultsStatus");

	private static final LoggedTunableNumber thetaStdDevCoefficient = group.build("thetaStdDevCoefficient", 0.075);
	private static final LoggedTunableNumber xyStdDevCoefficient = group.build("xyStdDevCoefficient", 0.075);

	private static final LoggedTunableNumber zHeightToleranceMeters = group.build("zHeightToleranceMeters", 0.5);
	private static final LoggedTunableNumber pitchAndRollToleranceDegrees = group.build("pitchToleranceDegrees", 10.0);

	private static final LoggedTunableNumber maxValidDistanceAwayFromCurrentEstimateMeters = group
			.build("MaxValidDistanceFromCurrentEstimateMeters", 30.0);
	private static final LoggedTunableNumber maxValidDistanceAwayFromCurrentHeadingDegrees = group
			.build("GyroFilteringToleranceDegrees", 30.0);
	...
	public Camera(CameraIO io, AprilTagFieldLayout aprilTagFieldLayout) {
		...
		this.tagsIdsOnField = io.getAprilTagFieldLayout().getTags().stream().map((tag) -> tag.ID)
				.collect(Collectors.toSet());

		this.missingCameraAlert = new Alert("Missing Camera: " + this.getCameraName(), Alert.AlertType.ERROR);
	}
	...
	/** Get the timestamp of the vision measurement in seconds. The timestamp has an epoch since FPGA time startup */
	public double getTimestampSeconds() {
		return inputs.timestampSecondsFPGA;
	}

	/**
	 * Get standard deviations of the vision measurements.
	 * Higher values numbers here means trust global measurements from this camera less.
	 * The matrix is in the form [x, y, theta], with units in meters and radians.
	 */
	public Matrix<N3, N1> getStandardDeviations() {

		// Get data about distance to each tag that is present on field
		DoubleSummaryStatistics distanceToTagsUsedSummary = Arrays.stream(inputs.tagsUsed)
				.mapToObj(aprilTagFieldLayout::getTagPose)
				.filter(Optional::isPresent).map(Optional::get)
				.mapToDouble(
						(tagPose3d) -> tagPose3d.getTranslation().getDistance(getEstimatedRobotPose().getTranslation()))
				.summaryStatistics();

		// This equation is a heuristic one, good enough but can probably be improved
		// Larger distances to tags and fewer observed tags result in higher uncertainty (larger standard deviations).
		// Average distance increases uncertainty expentainally while more tags decreases uncertainty linearly
		double standardDeviation = distanceToTagsUsedSummary.getCount() > 0
	...
	public VisionResultStatus getStatus() {
		...
		if (MathUtil.isNear(0, inputs.estimatedRobotPose.getRotation().getX(), pitchAndRollToleranceValueRadians) &&
				MathUtil.isNear(0, inputs.estimatedRobotPose.getRotation().getY(), pitchAndRollToleranceValueRadians)) {
