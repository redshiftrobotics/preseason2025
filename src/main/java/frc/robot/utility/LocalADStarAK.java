...
public class LocalADStarAK implements Pathfinder {
	...
	public boolean isNewPathAvailable() {
		if(!Logger.hasReplaySource()) { io.updateIsNewPathAvailable(); }
	 ...
	 *     obstacle it will be moved to the nearest non-obstacle node.
	 */
	@Override
	 ...
	 *     to change the start position of the path to properly avoid obstacles
	 */
	@Override
	...
	private static class ADStarIO implements LoggableInputs {
		...
		public void fromLog(LogTable table) {
			...
			double[] pointsLogged =
				table.get("CurrentPathPoints", new double[0]);
		...
		public void updateCurrentPathPoints(
			...
			PathPlannerPath currentPath =
				adStar.getCurrentPath(constraints, goalEndState);
