...
public class Alert {
	 ...
	 * to be instantiated, the appropriate entries will be added to
	 * NetworkTables.
	 *
	...
	/**
	 * Creates a new Alert. If this is the first to be instantiated in its
	 * group, the appropriate entries will be added to NetworkTables.
	 *
	...
	public Alert(String group, String text, AlertType type) {
		groups.computeIfAbsent(group, key -> {
	...
	private void reportAlert() {
		switch(type) {
			case ERROR: DriverStation.reportError(text, false); break;
			case WARNING: DriverStation.reportWarning(text, false); break;
			case INFO: System.out.println(text); break;
		}
	}
	...
	private static class SendableAlerts implements Sendable {
		...
		public String[] getStrings(AlertType type) {
			Predicate<Alert> activeFilter =
				(Alert x) -> x.type == type && x.active;
		...
		public void initSendable(SendableBuilder builder) {
			...
			builder.addStringArrayProperty(
				"errors", () -> getStrings(AlertType.ERROR), null);
			builder.addStringArrayProperty(
				"warnings", () -> getStrings(AlertType.WARNING), null);
			builder.addStringArrayProperty(
				"infos", () -> getStrings(AlertType.INFO), null);
		}
	}
	...
	public enum AlertType {
		...
		/**
		 * Medium priority alert - displayed second on the dashboard with a
		 * yellow "!" symbol. Use this type for problems which could affect the
		 * robot's functionality but do not necessarily require immediate
		 * attention.
		 */
		WARNING,
		 ...
		 * robot's functionality, or any other alerts which do not fall under
		 * "ERROR" or "WARNING".
		 */
		INFO
	}
